...
function buildRunId(now = new Date()) {
  return `headful-cloudflare-auth-handoff-${now.toISOString().replace(/[:.]/g, '-')}`;
}
...
function readDisplayMetadata(env = process.env) {
  const display = String(env.DISPLAY || '').trim() || ':99';
  const metadata = { display, wmctrlWindows: [], chromiumWindowIds: [] };
  ...
  try {
    const output = execFileSync('xdotool', ['search', '--onlyvisible', '--class', 'chromium'], {
      env: { ...process.env, DISPLAY: display },
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'],
    });
    metadata.chromiumWindowIds = output
      .split('\n')
      .map(line => line.trim())
      .filter(Boolean)
      .slice(0, 10);
...
function looksLikeCloudflareChallenge(state) {
  const url = String(state && state.url ? state.url : '');
  const title = String(state && state.title ? state.title : '');
  const bodyText = String(state && state.bodyText ? state.bodyText : '');
  return (
    /testchallengepage/i.test(url) ||
    /verify you are human|performing security verification|security verification|cloudflare/i.test(`${title}\n${bodyText}`)
  );
}

async function captureNamedScreenshot(page, screenshotDir, filename) {
  const screenshotPath = path.join(screenshotDir, filename);
  await page.screenshot({ path: screenshotPath, fullPage: true }).catch(() => {});
  return screenshotPath;
}

async function waitForManualCloudflareClear(page, { timeoutMs, pollMs, screenshotDir }) {
  const startedAt = new Date();
  const firstState = await capturePageState(page);
  if (!looksLikeCloudflareChallenge(firstState)) {
    return {
      appeared: false,
      cleared: false,
      operatorActionNeeded: false,
      waitedForHuman: false,
      reason: 'not_present',
  ...
  const screenshot = await captureNamedScreenshot(page, screenshotDir, '00-cloudflare-gate.png');
  ...
  while (Date.now() < deadline) {
    ...
    if (!looksLikeCloudflareChallenge(lastState)) {
      return {
        appeared: true,
        cleared: true,
        operatorActionNeeded: false,
        waitedForHuman: true,
        reason: 'manual_clear_detected',
        startedAt: startedAt.toISOString(),
  ...
  return {
    ...
    waitedForHuman: true,
    reason: 'manual_clear_timeout',
    startedAt: startedAt.toISOString(),
    clearedAt: null,
    screenshot,
    finalUrl: redactUrl(lastState.url),
  };
}
...
function parseHeaders(rawMessage) {
  ...
  for (const line of lines) {
    if (/^\s/.test(line) && folded.length) folded[folded.length - 1] += ` ${line.trim()}`;
...
class ImapSession {
  ...
  async command(command) {
    ...
    const finalLine = responseText.trim().split(/\r?\n/).pop();
...
async function fetchLatestBackMarketCode({ env = process.env, after, recipient = DEFAULT_PORTAL_EMAIL }) {
  const sinceDate = new Date(
    Math.max(Date.now() - 12 * 60 * 60 * 1000, new Date(after || Date.now()).getTime() - 10 * 60 * 1000)
  );
...
async function enterEmailCode(page, code) {
  ...
  if (!count) return { entered: false, reason: 'email_code_inputs_not_found' };
...
function buildMarkdownReport(result) {
  const lines = [
    '# Headful Cloudflare Auth Handoff Report',
    '',
    ...
    `- Cloudflare appeared: ${result.cloudflareAppeared ? 'yes' : 'no'}`,
    `- operator action needed: ${result.operatorActionNeeded ? 'yes' : 'no'}`,
    ...
    `- Cloudflare screenshot: \`${result.cloudflareScreenshot || 'not captured'}\``,
    ...
    `- final screenshot: \`${result.finalScreenshot || 'not captured'}\``,
    '',
    '## Final State',
    '',
    `- final redacted url: \`${result.finalUrl || 'n/a'}\``,
    `- display: \`${result.display || 'unknown'}\``,
    `- profile dir: \`${result.profileDir}\``,
    `- browser held open: ${result.browserHeldOpen ? 'yes' : 'no'}`,
    result.holdOpenUntil ? `- hold-open deadline: \`${result.holdOpenUntil}\`` : '- hold-open deadline: `n/a`',
    '',
    '## Operator Handoff',
    '',
    ...(result.operatorInstructions && result.operatorInstructions.length
      ? result.operatorInstructions.map(line => `- ${line}`)
      : ['- none']),
    '',
    '## Safety Confirmation',
    '',
...
async function holdBrowserOpen(page, holdOpenMs) {
  const until = new Date(Date.now() + holdOpenMs).toISOString();
  const startedAt = Date.now();
  while (Date.now() - startedAt < holdOpenMs) {
...
async function main() {
  ...
  const manualChallengeTimeoutMs = Number(argValue('--manual-challenge-timeout-ms', 10 * 60 * 1000));
  const manualChallengePollMs = Number(argValue('--manual-challenge-poll-ms', 5000));
  const holdOpenMs = Number(argValue('--hold-open-ms', 30 * 60 * 1000));
  const keepOpenOnBlock = !hasFlag('--no-keep-open-on-block');
  ...
  const screenshotDir = argValue(
    '--screenshot-dir',
    path.join(baseDataDir, 'screenshots', 'headful-cloudflare-auth-handoff', runId)
  );
  const runFile = argValue('--run-file', path.join(baseDataDir, 'runs', `${runId}.json`));
  const reportFile = argValue(
    '--report-file',
    path.join(__dirname, '..', 'REPORT-HEADFUL-CLOUDFLARE-AUTH-HANDOFF-2026-04-26.md')
  );
  ...
  const lock = acquireLock(lockPath, buildLockRecord({ operation: 'headful-cloudflare-auth-handoff' }));
  if (!lock.ok) throw new Error(`Runtime lock unavailable: ${lockPath}`);

  let context = null;
  let heldOpen = false;
  let holdOpenUntil = null;
  let exitCode = 1;
  try {
    ...
    const initialScreenshot = await captureNamedScreenshot(page, screenshotDir, '01-initial-state.png');

    let cloudflareStatus = await waitForManualCloudflareClear(page, {
      timeoutMs: manualChallengeTimeoutMs,
      pollMs: manualChallengePollMs,
      screenshotDir,
    });
    let currentState = cloudflareStatus.cleared ? await capturePageState(page) : initialState;
    let portalState = detectPortalState(currentState);
    ...
    if (!cloudflareStatus.cleared && cloudflareStatus.appeared) {
      operatorInstructions = [
        `Attach to the existing GUI session on display ${readDisplayMetadata().display}.`,
        'Bring the Chromium window for this Back Market session to the front.',
        'Complete only the Cloudflare human verification if it is visible.',
        'Do not navigate elsewhere, open listings, or perform any portal actions after the challenge clears.',
        `The browser is using the dedicated profile at ${profileDir}.`,
      ];
    } else {
      if (portalState.blocker === 'email_entry_required') {
        ...
        postEmailScreenshot = await captureNamedScreenshot(page, screenshotDir, '02-post-email-submit.png');
        currentState = postEmailState;
        finalScreenshot = postEmailScreenshot;
        portalState = detectPortalState(currentState);

        if (looksLikeCloudflareChallenge(currentState)) {
          cloudflareStatus = await waitForManualCloudflareClear(page, {
            timeoutMs: manualChallengeTimeoutMs,
            pollMs: manualChallengePollMs,
            screenshotDir,
          });
          currentState = cloudflareStatus.cleared ? await capturePageState(page) : currentState;
          portalState = detectPortalState(currentState);
        }
      }
      ...
      if (portalState.blocker === 'password_required') {
        ...
        postPasswordScreenshot = await captureNamedScreenshot(page, screenshotDir, '03-post-password-submit.png');
        currentState = postPasswordState;
      ...
      if (portalState.blocker === 'email_code_required') {
        codePromptScreenshot = await captureNamedScreenshot(page, screenshotDir, '04-email-code-prompt.png');
        finalScreenshot = codePromptScreenshot;
        mailboxResult = await fetchLatestBackMarketCode({
          env: process.env,
          after:
            passwordSubmission && passwordSubmission.submittedAt
              ? new Date(new Date(passwordSubmission.submittedAt).getTime() - 120000).toISOString()
        ...
        if (mailboxResult.codeFound && mailboxResult.code) {
          ...
          currentState = await capturePageState(page);
          finalScreenshot = await captureNamedScreenshot(page, screenshotDir, '05-post-code-dashboard.png');
          portalState = detectPortalState(currentState);
        }
      }

      if (portalState.state === 'logged_in' && !/dashboard|listings|bo-seller|bo_merchant/i.test(page.url())) {
        await page.goto(listingsUrl, { waitUntil: 'domcontentloaded', timeout: timeoutMs }).catch(() => null);
        await waitForSettledPage(page, 2500);
        currentState = await capturePageState(page);
        finalScreenshot = await captureNamedScreenshot(page, screenshotDir, '06-listings-landing.png');
        portalState = detectPortalState(currentState);
      }
    }

    const displayMetadata = readDisplayMetadata();
    const finalState = currentState || initialState;
    const blockedForOperator = Boolean(cloudflareStatus.appeared && !cloudflareStatus.cleared);
    ...
    const result = {
      ...
      reportFile,
      profileDir,
      display: displayMetadata.display,
      wmctrlWindows: displayMetadata.wmctrlWindows,
      chromiumWindowIds: displayMetadata.chromiumWindowIds,
      finalState: portalState.state,
      finalUrl: redactUrl(finalState.url),
      blocker: portalState.state === 'logged_in' ? null : blockedForOperator ? 'cloudflare_manual_handoff_required' : portalState.blocker || 'unknown',
      cloudflareAppeared: Boolean(cloudflareStatus.appeared),
      operatorActionNeeded,
      dashboardReached: portalState.state === 'logged_in',
      browserHeldOpen: false,
      holdOpenUntil: null,
      portalEmail,
      ...
      challenge: {
        appeared: Boolean(cloudflareStatus.appeared),
        cleared: Boolean(cloudflareStatus.cleared),
        waitedForHuman: Boolean(cloudflareStatus.waitedForHuman),
        reason: cloudflareStatus.reason,
        startedAt: cloudflareStatus.startedAt,
        clearedAt: cloudflareStatus.clearedAt || null,
      },
      ...
      screenshots: {
        initialScreenshot,
        cloudflareScreenshot: cloudflareStatus.screenshot,
        postEmailScreenshot,
    ...
    if (blockedForOperator && keepOpenOnBlock) {
      result.browserHeldOpen = true;
      ...
      heldOpen = true;
      holdOpenUntil = await holdBrowserOpen(page, holdOpenMs);
      result.holdOpenUntil = holdOpenUntil;
    }
      ...
      JSON.stringify(
        {
          ...
          cloudflareAppeared: result.cloudflareAppeared,
          operatorActionNeeded: result.operatorActionNeeded,
          reportFile,
          runFile,
          browserHeldOpen: result.browserHeldOpen,
          holdOpenUntil: result.holdOpenUntil,
        },
        null,
        2
      )
    ...
    if (context && !heldOpen) await context.close().catch(() => {});
    if (heldOpen && context) await context.close().catch(() => {});
