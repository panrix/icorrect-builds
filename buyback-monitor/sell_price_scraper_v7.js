 ...
 * 
 * Usage:
 *   node sell_price_scraper_v7.js                    # MacBook only
 *   node sell_price_scraper_v7.js --iphone-ipad      # MacBook + iPhone/iPad
 *   node sell_price_scraper_v7.js --all              # All devices (same as --iphone-ipad)
 *   node sell_price_scraper_v7.js --dry-run           # Show catalogue, don't scrape
 *   node sell_price_scraper_v7.js --model "Pro 14"    # Filter to matching models
 *   node sell_price_scraper_v7.js --model "iPhone"    # iPhone models only
 */
...
const IPHONE_IPAD_CATALOGUE_PATH = path.join(BASE_DIR, 'config', 'scrape-urls-iphone-ipad.json');
const DATA_DIR = path.join(BASE_DIR, 'data');

// Anti-detection: jitter and context rotation
const JITTER_MIN_MS = 3000;  // 3 seconds minimum delay between model scrapes
const JITTER_MAX_MS = 8000;  // 8 seconds maximum delay between model scrapes
const BACKFILL_JITTER_MIN_MS = 5000;  // 5 seconds minimum delay during backfill pass
const BACKFILL_JITTER_MAX_MS = 12000; // 12 seconds maximum delay during backfill pass

// Telegram alerting
const ENV_PATH = '/home/ricky/config/.env';
const ALERT_THRESHOLD = 0.9; // Alert if success rate below 90%
...
function randomBackfillJitter() {
  return BACKFILL_JITTER_MIN_MS + Math.random() * (BACKFILL_JITTER_MAX_MS - BACKFILL_JITTER_MIN_MS);
}

function nextRotationInterval() {
  return 4 + Math.floor(Math.random() * 2); // 4 or 5
}

async function sleepWithLog(ms, label) {
  console.log(`  Waiting ${(ms / 1000).toFixed(1)}s ${label}...`);
  await new Promise(r => setTimeout(r, ms));
}

/**
 * Create a fresh browser context with homepage warmup.
 */
...
/**
 * Load env var from /home/ricky/config/.env
 */
function loadEnvVar(name) {
  ...
  } catch (e) { /* env file not found */ }
  return null;
}

/**
 * Send a Telegram alert via HTTPS POST (no external deps).
 */
function sendTelegramAlert(message) {
  return new Promise((resolve) => {
    const token = loadEnvVar('TELEGRAM_BOT_TOKEN');
    const chatId = loadEnvVar('BM_TELEGRAM_CHAT');
    if (!token) {
      console.log('  ⚠ No TELEGRAM_BOT_TOKEN found, skipping alert');
      return resolve();
    }
    if (!chatId) {
      console.log('  ⚠ No BM_TELEGRAM_CHAT found, skipping alert');
      return resolve();
    }
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ...
          console.log(`  ⚠ Telegram alert failed (HTTP ${res.statusCode}): ${data.slice(0, 200)}`);
        }
        resolve();
      });
    });
...
async function main() {
  ...
  let context = await createWarmContext(browser);

  const results = {};
  let scrapedOk = 0;
  let failed = 0;
  let totalPickers = 0;
  let scrapedInContext = 0; // Track models scraped with current context
  let currentRotationInterval = nextRotationInterval();
  const failedEntries = []; // Track failures for backfill pass
  ...
  for (let i = 0; i < catalogue.length; i++) {
    ...
    // Context rotation: fresh browser context every N models
    if (scrapedInContext >= currentRotationInterval) {
      console.log(`\n--- Rotating browser context (${scrapedInContext} models scraped in batch; next batch target ${currentRotationInterval}) ---`);
      await context.close();
      context = await createWarmContext(browser);
      scrapedInContext = 0;
      currentRotationInterval = nextRotationInterval();
    }
    ...
    // Jitter delay between pages (not after last)
    if (i < catalogue.length - 1) {
      await sleepWithLog(randomJitter(), 'before next model');
    }
  }

  // --- Backfill pass for failed models ---
  if (failedEntries.length > 0) {
    console.log(`\n=== BACKFILL PASS: Starting backfill pass for ${failedEntries.length} failed model(s) ===`);
    await context.close();
    context = await createWarmContext(browser);
    let recovered = 0;
    ...
    for (let i = 0; i < failedEntries.length; i++) {
      ...
      console.log(`\n[Backfill ${i + 1}/${failedEntries.length}] Retrying ${entry.model}...`);
      ...
      if (result.error) {
        ...
        // Backfill succeeded — update results
        console.log(`  ✓ Backfill succeeded for ${entry.model}`);
        scrapedOk++;
        failed--;
        recovered++;
        totalPickers += result.totalPickers;
      ...
      // Jitter between backfill scrapes (not after last)
      if (i < failedEntries.length - 1) {
        await sleepWithLog(randomBackfillJitter(), 'before next backfill model');
      }
    }

    console.log(`\n=== Backfill complete: ${recovered} recovered, ${failed} still failed ===`);
  }
  ...
  // Telegram alert if success rate below threshold
  const successRate = scrapedOk / catalogue.length;
  if (successRate < ALERT_THRESHOLD) {
    const failedNames = Object.entries(results)
      .filter(([_, v]) => v.scraped === false && v.error)
      .map(([name]) => name);
    const alertMsg = `⚠️ *V6 Scraper Alert*\n${scrapedOk}/${catalogue.length} models scraped (${(successRate * 100).toFixed(0)}%)\nFailed: ${failedNames.join(', ')}`;
    console.log(`\nSending Telegram alert (success rate ${(successRate * 100).toFixed(0)}% < ${(ALERT_THRESHOLD * 100).toFixed(0)}% threshold)...`);
