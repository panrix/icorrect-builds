...
Variant integrity uses a two-tier precedence: **canonical resolver truth first**, then raw catalog as advisory fallback.

#### Tier 1: Resolver-managed listings

Look up the listing in `data/listings-registry.json` by listing ID, SKU, or product ID. If a live-safe resolver slot exists (`trust_class` is `registry_verified`, `probe_verified`, or `vetted_exact_with_no_current_contradiction`):

- The resolver slot is the authoritative source for colour, grade, and product identity
- Compare resolver truth against the linked Monday item:
  - colour: Main Board `status8` vs resolver `colour` (normalized)
  - grade: Main Board `status_2_Mjj4GJNQ` vs live listing grade (mapped to BM grade)
  - product_id: live listing vs resolver `product_id`
  - SKU: live listing vs resolver `sku`
- If any mismatch:
  - flag as critical
  - set BM listing `quantity` to `0` immediately
  - alert BM Telegram

#### Tier 2: Unmanaged listings (advisory only)

If no live-safe resolver slot exists, fall back to raw `bm-catalog.json`. But this is **advisory only**:

- Compare catalog variant against the linked Monday item (same checks as before)
- If a mismatch is found:
  - flag as a warning alert
  - **do NOT auto-offline** — raw catalog may be stale
  - alert BM Telegram with "ALERT ONLY" prefix
- If the listing passes advisory checks, note it as "unmanaged" in the report

**Why this change:** The resolver truth store may have corrected data that diverges from the raw catalog (e.g. BM 1533 where catalog says Starlight but the UUID is actually used for Midnight). Auto-offlining based on stale catalog data would take down valid listings.

#### Auto-bump gate

`--auto-bump` only applies price changes to **resolver-managed** listings where `variantCheck.managed === true`. Unmanaged listings are never auto-bumped.
...
### Resolver Truth + Catalog
| Source | Purpose |
|--------|---------|
| `backmarket/data/listings-registry.json` | Canonical resolver truth store — authoritative for managed listings |
| `backmarket/data/bm-catalog.json` | Advisory fallback for unmanaged listings |
| `backmarket/scripts/lib/resolver-truth.js` | Shared resolver module (lookup, normalization, trust policy) |
...
- No bumps on unmanaged listings (must have a live-safe resolver slot)
- No auto-offline of unmanaged listings based on raw catalog mismatch alone
- Resolver-managed listings with variant mismatch are still auto-offlined
...
3. **RESOLVED — Explicit `--dry-run` flag exists**

`buy-box-check.js` accepts `--dry-run`. Running without `--auto-bump` is still check-only, but the named dry-run flag is available and should be preferred when you want to suppress both mutations and Telegram posts.
...
- Explicit `--dry-run` flag exists and suppresses live mutations and Telegram posts.
