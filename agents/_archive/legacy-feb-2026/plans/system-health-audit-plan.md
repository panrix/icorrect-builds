...
## Step 0: Consolidate All Agent Build Docs into `/builds/agents/`

Create `/home/ricky/builds/agents/` and consolidate all scattered agent build docs into one place.

### Files to copy into `/builds/agents/`:

**From VPS Jarvis workspace (`~/.openclaw/agents/main/workspace/docs/`):**
- `agent-architecture-map.md` — org chart, agent IDs, Telegram mappings
- `code-master-brief.md` — Jarvis's 11 issues (the reference doc)
- `code-agent-brief.md` — code quality issues
- `data-architecture-brief.md` — data flow, Supabase schema
- `token-usage-optimisation.md` — cost analysis
- `subagent-context.md` — sub-agent reference

**From VPS Mission Control v2 (`~/mission-control-v2/docs/`):**
- `PRD.md` — official build spec
- `PRD-HANDOFF.md` — handoff summary
- `BUILD.md` — deployment/infrastructure docs
- `BUILD-PLAN.md` — 12-phase roadmap + checklist
- `FINANCE-MERGE.md` — finance→operations merge
- `MC-BUILD-BRIEF.md` — build overview
- `qa-trigger/MASTER-AUDIT.md` — QA audit (single source of truth)
- `qa-trigger/QA-TRIGGER-PLAN.md` — QA pipeline architecture

**From VPS Cursor plans (`~/.cursor/plans/`):**
- `system_health_assessment_e5f33bb3.plan.md` — Cursor health audit plan
- `system_health_audit_plan_27f675e6.plan.md` — alternative audit plan

**From VPS home (`~/`):**
- `system-health-audit-plan.md` — this plan (already there)

**From local Mac (`~/.claude/plans/`):**
- `rippling-wibbling-mist.md` — this plan file
- `frolicking-conjuring-kettle.md` — original MC v2 plan (referenced in BUILD-PLAN)
- `calm-noodling-hollerith.md` — agent build plan (Feb 18)
- `lovely-bouncing-narwhal.md` — agent build plan (Feb 22)
- `modular-mapping-rabin.md` — agent architecture plan (Feb 18)
- `tranquil-spinning-dolphin.md` — VPS security audit plan

### Directory structure:
```
/home/ricky/builds/agents/
├── system-health-audit-plan.md    (this plan — master reference)
├── qa-trigger/
│   ├── MASTER-AUDIT.md
│   └── QA-TRIGGER-PLAN.md
├── plans/                         (all historical plans)
│   ├── cursor-health-assessment.plan.md
│   ├── cursor-health-audit.plan.md
│   └── (local Claude plans)
└── (all other docs flat)
```
